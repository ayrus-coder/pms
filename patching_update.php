...
?>
<a class='btn btn-info btn-lg' style='width:200px;margin-top:5px;' href="http://<?php echo $gw_host; ?>/patch/show.php?log=<?php echo $result['logfile']; ?>"><span class="glyphicon glyphicon-file"><br/>Log</span></a>
<?php 
} 
else {
...
if( $result['status'] == 'QUEUED') {
  ...
  echo " <a class='btn btn-danger btn-lg' style='width:200px;margin-top:5px;'  href='patching_window_change.php?mid=".$result['maintenance_requests_id']."&id=".$result['id']."'><span class='glyphicon glyphicon-edit'><br/>Reschedule</span> </a>";
if(($result['coordination'] == 'Y' && $result['coord_mail'] == 'Y') or ($result['coordination'] == 'N')){
?>
...
?>
<a class='btn btn-primary btn-lg' style='width:200px;margin-top:5px;' href="patching_status.php?mid=<?php echo $_result['maintenance_requests_id']; ?>&id=<?php echo $result['id']; ?>&status=COMPLETED" ><span class="glyphicon glyphicon-ok" ><br/>Completed </span></a>
<?php
}
else {
if($result['mins'] < 180) {
