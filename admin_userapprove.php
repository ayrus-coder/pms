...
try {
    ...
    $sql = "select uid from maintenance_requests where uid=$uid and (maintenance_requests.to  like '%$user%' or cc like '%$user%')";
...
function display_approve_request ($uid){
...
try {
    $conn = new PDO("mysql:host=$servername;dbname=patch", $username, $password);
    $sql = "select id, hostname, maintenance_requests.to, cc, approved, status, postponed, updated_date, DATE_FORMAT(scheduled_date, '%Y/%c/%d %H:%m:%s') as scheduled_date, approved_by, approved_at, last_patched_date,comment from maintenance_requests where uid=$uid";
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $edit = false;
...
return $count;
  ...
  if(validate_user($user, $uid) == false) {
?>
<div class="container">
<div class="panel panel-primary">
  <div class="panel-heading"> Approval Update</span></div>
  <div class="panel-body text-center">
    <h2><span class='glyphicon glyphicon-warning-sign' style='color:#f0ad4e;'></span> </h2>
   <h2>Your not authorised to access this page or no entry exist for this requested ID!</h2>
   <div>
<div>
<div>
<?php
...
<div class="panel panel-body">
<?php
$count = display_approve_request($_REQUEST['uid']);
?>
</div>
</div>
</form>
